...
export function ReadinessGauge({
  ...
  return (
    ...
    >
      <span className="relative h-1.5 overflow-hidden rounded-full bg-surface-2" aria-hidden>
        <span
          className={classNames("absolute inset-y-0 left-0 rounded-full", fill)}
          style={{ width: `${value}%` }}
        />
      </span>
      <span className="text-right font-mono text-[11px] tabular-nums text-ink-strong">{value}</span>
    </span>
  );
}
