...
function QueryBarRoot({ children, className, ariaLabel = "Filter bar" }: QueryBarProps) {
  return (
    <div
      role="search"
...
function QueryBarDivider() {
  return <span aria-hidden className="mx-0.5 hidden sm:block h-5 w-px shrink-0 bg-border" />;
}
...
function QueryBarUtility({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={classNames("flex items-center gap-0.5 shrink-0 pr-1", className)}>
      {children}
    </div>
  );
}
...
function QueryBarFilterTrigger(props: QueryBarFilterTriggerProps) {
  const { label, options, placeholder = "Any", icon, align = "start", className } = props;
  const id = useId();
  const [open, setOpen] = useState(false);

  const selected: string[] = props.multi ? props.value : props.value ? [props.value] : [];
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align={align} sideOffset={6} className="w-64 p-0 border-border bg-popover">
        <Command>
  ...
  return (
    ...
    >
      <span aria-live="polite">
        ...
        {showTotal ? <span className="opacity-70"> of {total!.toLocaleString()}</span> : null}{" "}
        {noun}
      </span>
      {activeCount > 0 ? (
        <>
