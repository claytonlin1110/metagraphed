...
export function Chip({
  ...
  return (
    ...
    >
      {dot ? (
        <span aria-hidden className="mg-health-dot" style={{ color: "currentColor" }} />
      ) : icon ? (
        <span aria-hidden className="inline-flex size-3 items-center justify-center">
          {icon}
        </span>
      ) : null}
      {label ? <span className="uppercase tracking-widest opacity-70">{label}</span> : null}
      {children != null ? (
        <span className="text-ink-strong normal-case tracking-normal">{children}</span>
      ) : null}
    </Cmp>
  );
}
