 ...
 */
export function DefinitionList({ items, layout = "inline", className }: DefinitionListProps) {
  if (layout === "grid") {
    return (
      <dl className={cn("grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2", className)}>
        {items.map((it, i) => (
          <div key={i} className="min-w-0">
            ...
            <dd className="mt-1 truncate text-sm text-ink-strong">{it.detail}</dd>
          </div>
        ))}
      </dl>
    );
  ...
  return (
    <dl className={cn("divide-y divide-border/70", className)}>
      {items.map((it, i) => (
        ...
        >
          <dt title={it.title} className="mg-type-label shrink-0 text-ink-muted">
            {it.term}
          </dt>
          <dd className="min-w-0 truncate text-right text-sm text-ink-strong">{it.detail}</dd>
        </div>
      ))}
    </dl>
  );
