...
import { AlertTriangle, ExternalLink, Filter, Inbox, RotateCcw } from "lucide-react";
import { classNames } from "@/lib/metagraphed/format";

export type EmptyStateVariant = "empty" | "filtered" | "error" | "stale";

const VARIANT_ICON: Record<EmptyStateVariant, ComponentType<{ className?: string }>> = {
  empty: Inbox,
  filtered: Filter,
  error: AlertTriangle,
  stale: RotateCcw,
  ...
  return (
    ...
    >
      ...
      <div className="max-w-sm space-y-1">
        <p className="font-display text-[15px] font-medium text-ink-strong">{title}</p>
        {hint != null ? <p className="text-[13px] leading-relaxed text-ink-muted">{hint}</p> : null}
      </div>
