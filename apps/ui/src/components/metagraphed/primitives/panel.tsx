    ...
    >
      {hasHeader ? (
        <header
          ...
          style={{ paddingTop: "var(--mg-space-sm)", paddingBottom: "var(--mg-space-sm)" }}
        >
          <div className="min-w-0">
            {title != null ? <SectionLabel>{title}</SectionLabel> : null}
            {caption != null ? <p className="mt-1 text-[13px] text-ink-muted">{caption}</p> : null}
          </div>
          {action != null ? <div className="shrink-0 flex items-center gap-2">{action}</div> : null}
        </header>
      ) : null}
